...
scene1 = scene
  where
    ...
    light2 = LS.PointLight 550 (Rgb 1 0.7 1) (0, 0, 30)
    s3 = addLightSrc (addLightSrc s2 light) light2
    s4 = foldl addMesh s3 smallBallMeshes
    s5 = addLightSrc s4 $ LS.AmbientLight 0.3 white
    scene = foldl addMesh s5 bigBallMeshes
...
sampleScenes :: [Scene]
sampleScenes = [scene1, scene2, scene3, scene4, scene5]
...
main = do
  ...
  let p = perspective2 imageWidth imageHeight in
